...
const initialPeople = [
  { name: 'Sarah Chen', role: 'Frontend Developer', status: 'Active', color: 'success', avatar: 'S' },
  { name: 'Michael Rodriguez', role: 'Backend Engineer', status: 'Active', color: 'success', avatar: 'M' },
  { name: 'Emily Thompson', role: 'UI/UX Designer', status: 'Active', color: 'success', avatar: 'E' },
  { name: 'David Kim', role: 'Project Manager', status: 'Active', color: 'success', avatar: 'D' },
]
...
export default function TeamCollaboration() {
  ...
  const toDisplay = (m, projectCount = 1) => {
    ...
    // Human-friendly subtitle without duplicated wording
    const subtitle = projectCount > 1 ? `Working on ${projectCount} projects` : 'Project member'
    // Preserve potential unique identifiers for stable React keys
    const id = m._id || m.id
    const email = m.email
    const key = id || email || m.username || m.name || name
    return { id, email, key, name, avatar, status, color, subtitle }
  }
  ...
  const fetchTeam = useCallback(async () => {
    ...
    try {
      ...
      if (!selectedProjectId && sampleProjects.length > 0) {
        const firstId = sampleProjects[0]._id || sampleProjects[0].id
        if (firstId) setSelectedProjectId(firstId)
      }
      const counts = new Map()
      const byKey = new Map()
  ...
  await inviteMember(email, selectedProjectId)
      ...
      if (!already) {
        ...
        setTeam((prev) => [
          ...prev,
          { name: email, email, key: email, subtitle: 'Invite pending', status: 'Pending', color: 'default', avatar: initial },
        ])
      }
  ...
  <Stack spacing={0.75}>
        ...
        {!loading && !error && team.map((p, idx) => (
          <Box key={p.id || p._id || p.email || p.key || `${p.name}-${idx}`} sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 2, px: 1, py: 0.75, borderRadius: 2.5, bgcolor: 'grey.50', border: '1px solid', borderColor: 'divider' }}>
