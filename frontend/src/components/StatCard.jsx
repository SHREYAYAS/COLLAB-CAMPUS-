...
export default function StatCard({
  ...
  return (
  <Paper className={className} sx={{ p: 2.5, display: 'flex', flexDirection: 'column', borderRadius: 3, border: '1px solid', borderColor: 'divider', boxShadow: 'none', ...sx }}>
      <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 1 }}>
        <Typography variant="h6" sx={{ color: titleColor }}>{title}</Typography>
        <Box sx={{ display: 'inline-flex', alignItems: 'center', gap: 1 }}>
          {icon && (
            <Box sx={{ bgcolor: '#f1f3ff', borderRadius: 2, p: 1, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', ...iconBoxSx }}>
              {icon}
            </Box>
          )}
          {showCornerArrow && (
      ...
      <Typography variant="h3" fontWeight="bold" sx={{ color: valueColor || 'text.primary', letterSpacing: '-0.02em' }}>{value}</Typography>
        ...
        <Box sx={{ mt: 1 }}>
          <Box
            ...
            sx={{
              ...
              fontSize: 12,
              fontWeight: 600,
              bgcolor: badgeBg || (trend === 'down' ? '#fde8e8' : '#e8fff4'),
